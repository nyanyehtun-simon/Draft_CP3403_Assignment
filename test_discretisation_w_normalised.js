//THIS HELPS TO NORMALISE IN [0, 1]:
...
var normalise = function (aNum, max){
    var toReturn = aNum;
    toReturn = (toReturn * 1)/max;
    return toReturn;
};
...
let MAX_NORMALISED = 1;
let MIN_NORMALISED = 0;
let RANGE_NORMALISED = 1;
    ...
    for (i = numListMin; i <= numListMax; i += binSize) {
        ...
        data.forEach((object => {
            if (object[key] == numListMax){
                oldValue = object[key];
                data[n][key] = "From " + normalise(numListMax - binSize, numListMax) + " to " + normalise(numListMax, numListMax);
                console.log(oldValue + " (normalised as " + normalise(oldValue, numListMax)+") " + " goes to " + object[key]);
            } else if ( object[key] == numListMin ){
                oldValue = object[key];
                data[n][key] = "From " + normalise(numListMin, numListMax) + " to " + normalise(numListMin + binSize, numListMax);
                console.log(oldValue + " (normalised as " + normalise(oldValue, numListMax)+") " + " goes to " + object[key]);
            } else if (object[key] >= i && object[key] < (i+binSize)){
                oldValue = object[key];
                data[n][key]= "From " + normalise(i, numListMax) + " to " + normalise(i+binSize, numListMax);
                console.log(oldValue + " (normalised as " + normalise(oldValue, numListMax)+") " +" goes to " + object[key]);
            }
            n++;
        }));
